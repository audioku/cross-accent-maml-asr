...
def pad_list_with_mask(xs, pad_value):
    # From: espnet/src/nets/e2e_asr_th.py: pad_list()
    n_batch = len(xs)
    max_len = max([x.size(0) for x in xs])
    pad = xs[0].new(n_batch, max_len, * xs[0].size()[1:]).fill_(pad_value)
...
class FactorizedPositionwiseFeedForward(nn.Module):
    ...
    FFN(x) = max(0, xW1 + b1) W2+ b2
    """
    def __init__(self, dim_model, dim_ff, dropout=0.1, r=100):
        super(FactorizedPositionwiseFeedForward, self).__init__()
        self.linear_1_a = nn.Linear(dim_model, r, bias=False)
        self.linear_1_b = nn.Linear(r, dim_ff)
        self.linear_2_a = nn.Linear(dim_ff, r, bias=False)
        self.linear_2_b = nn.Linear(r, dim_model)
        self.dropout = nn.Dropout(dropout)
        self.layer_norm = nn.LayerNorm(dim_model)
    ...
    def forward(self, x):
        ...
        """
        residual = x
        output = self.dropout(self.linear_2_b(self.linear_2_a(F.relu(self.linear_1_b(self.linear_1_a(x))))))
        output = self.layer_norm(output + residual)
        return output
...
class FactorizedMultiHeadAttention(nn.Module):
    def __init__(self, num_heads, dim_model, dim_key, dim_value, dropout=0.1, r=100):
        ...
        self.query_linear_a = nn.Linear(dim_model, r, bias=False)
        self.query_linear_b = nn.Linear(r, num_heads * dim_key)
        
        self.key_linear_a = nn.Linear(dim_model, r, bias=False)
        self.key_linear_b = nn.Linear(r, num_heads * dim_key)
        
        self.value_linear_a = nn.Linear(dim_model, r, bias=False)
        self.value_linear_b = nn.Linear(r, num_heads * dim_value)

        nn.init.normal_(self.query_linear_a.weight, mean=0, std=np.sqrt(2.0 / (self.dim_model + self.dim_key)))
        nn.init.normal_(self.query_linear_b.weight, mean=0, std=np.sqrt(2.0 / (self.dim_model + self.dim_key)))
        nn.init.normal_(self.key_linear_a.weight, mean=0, std=np.sqrt(2.0 / (self.dim_model + self.dim_key)))
        nn.init.normal_(self.key_linear_b.weight, mean=0, std=np.sqrt(2.0 / (self.dim_model + self.dim_key)))
        nn.init.normal_(self.value_linear_a.weight, mean=0, std=np.sqrt(2.0 / (self.dim_model + self.dim_value)))
        nn.init.normal_(self.value_linear_b.weight, mean=0, std=np.sqrt(2.0 / (self.dim_model + self.dim_value)))

        self.attention = ScaledDotProductAttention(temperature=np.power(dim_key, 0.5), attn_dropout=dropout)
        self.layer_norm = nn.LayerNorm(dim_model)

        self.output_linear_a = nn.Linear(num_heads * dim_value, r, bias=False)
        self.output_linear_b = nn.Linear(r, dim_model)
        nn.init.xavier_normal_(self.output_linear_a.weight)
        nn.init.xavier_normal_(self.output_linear_b.weight)
    ...
    def forward(self, query, key, value, mask=None):
        ...
        query = self.query_linear_b(self.query_linear_a(query)).view(batch_size, len_query, self.num_heads, self.dim_key) # B x T_Q x num_heads x H_K
        key = self.key_linear_b(self.key_linear_a(key)).view(batch_size, len_key, self.num_heads, self.dim_key) # B x T_K x num_heads x H_K
        value = self.value_linear_b(self.value_linear_a(value)).view(batch_size, len_value, self.num_heads, self.dim_value) # B x T_V x num_heads x H_V
        ...
        output = self.dropout(self.output_linear_b(self.output_linear_a(output))) # B x T_Q x H_O
        output = self.layer_norm(output + residual)
