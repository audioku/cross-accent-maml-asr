...
class MultiSpectrogramDataset(Dataset, SpectrogramParser):
    def __init__(self, audio_conf, manifest_filepath_list, label2id, normalize=False, augment=False, lang_list=[]):
        """
    ...
    def __getitem__(self, index):
        ...
        for i in range(len(self.ids_list)):
            ...
            if len(self.lang_list) == len(self.ids_list):
                transcript = self.parse_transcript(transcript_path, add_suffix=self.lang_list[i])
            else:
    ...
    def parse_transcript(self, transcript_path, add_suffix=""):
        ...
        # add language suffix
        if add_suffix != "":
            transcript = [self.label2id.get(add_suffix)] + transcript
