...
if __name__ == '__main__':
    ...
    # add lang token
    if args.lang_list is not None:
        if len(args.lang_list) == len(args.train_manifest_list):
            lang_list = args.lang_list
            print("add language token")
            for i in range(len(lang_list)):
                label2id[lang_list[i]] = len(label2id)
                id2label[len(id2label)] = lang_list[i]
