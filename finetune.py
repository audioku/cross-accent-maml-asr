...
parser = argparse.ArgumentParser(description='Finetuning ASR training')
...
parser.add_argument('--training-mode', default='meta', type=str, help='')
...
if __name__ == '__main__':
    ...
    assert args.continue_from == ""

    if not os.path.exists("./log"): os.mkdir("./log")
    for handler in logging.root.handlers[:]: logging.root.removeHandler(handler)
        
    logging.basicConfig(filename="log/finetune_" + args.name + ".log", filemode='a+', format='%(asctime)s - %(message)s', level=logging.INFO)
